...
import { createAnnouncement, fetchAnnouncements } from "@/services/apis/announcment.api";
...
function CreateAnnouncment() {
    ...
    const handleSubmit = async () => {
        const { success, response } = await createAnnouncement({ content: announcementText });
        if (!success) return toast.error(response);
        toast.success("Announcement created successfully");
        queryClient.invalidateQueries({ queryKey: ["fetchAnnouncements"] });
    };

    return <div className="grid grid-cols-4 rounded-[1em] gap-[1.5em]">
        <Input
...
function Announcements() {
    ...
    return (<div className="flex flex-col w-full rounded-[1em] gap-[1.5em] p-[2em] bg-white shadow-lg">
        ...
        <div className="flex flex-col gap-[2em] w-full">
            {data?.map((announcement:any, index:number) => (
                <div className="flex flex-col w-full bg-gray-100 p-[1em] rounded-[1em] shadow-sm" key={index}>
                    ...
                    {formatDate(announcement.createdAt)}                    </p>
